...
- Container startup runs a one-time non-interactive `openclaw onboard --accept-risk` using the first available provider key.
  A marker file is stored at `<OPENCLAW_STATE_DIR>/.onboarded` to avoid re-running on restarts.
